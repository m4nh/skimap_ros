                                   ...
                                   const std::vector<std::pair<unsigned, unsigned>> &ref_query_ids)
{
  ...
  for (; it < current_max_iterations; ++it)
  {
    ...
    for (unsigned rqidx = 0; rqidx < no_of_matches; ++rqidx)
      if ((current_transformation * query[ref_query_ids[rqidx].second].cast<double>() - reference[ref_query_ids[rqidx].first].cast<double>()).squaredNorm() <= m_squared_threshold)
  ...
  for (unsigned rqidx = 0; rqidx < no_of_matches; ++rqidx)
    if ((m_query2ref * query[ref_query_ids[rqidx].second].cast<double>() - reference[ref_query_ids[rqidx].first].cast<double>()).squaredNorm() <= m_squared_threshold)
