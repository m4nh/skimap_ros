...
bool slamdunk::FeatureTracker::track(const RGBDFrame& frame)
{
  ...
  if(m_verbose) std::cout << SLAM_DUNK_INFO_STR(rqi_inliers.size() << " inliers survived") << std::endl;
#ifndef NDEBUG
  if(m_debug)
  {
...
void slamdunk::FeatureTracker::extractFrameData(const RGBDFrame& frame, FrameData& frame_data, std::vector<cv::KeyPoint>* kpts_ptr) const
{
  ...
  if(m_verbose) std::cout << SLAM_DUNK_INFO_STR(cvdesc.rows << " features extracted") << std::endl;
#ifndef NDEBUG
  if(m_debug)
  {
