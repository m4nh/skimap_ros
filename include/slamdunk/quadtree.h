...
namespace slamdunk
{
  ...
  class SLAM_DUNK_API QuadTreeBranch : public QuadTreeNode<DataTp, typename LeafTp::ElementRefTp>
  {
    public:
      ...
      virtual ~QuadTreeBranch() { delete m_children[0]; delete m_children[1]; delete m_children[2]; delete m_children[3]; }
      ...
      virtual inline ElementRef insert(Element& e)
      {
        ...
        if(e.m_coords.x() >= 1.0) e.m_coords.x() -= 1.0;
        if(e.m_coords.y() >= 1.0) e.m_coords.y() -= 1.0;
  ...
  class SLAM_DUNK_API QuadTreeLeafElementRef
  {
    public:
      ...
      bool erase()
      {
        if(!nodePtr || elemIt == nodePtr->m_elements.end()) return false;
